...
const [styles, analytics, questions, storage, markdown, ai, app] = await Promise.all([
  read('styles.css'), read('src/analytics.js'), read('src/questions.js'), read('src/storage.js'), read('src/markdown.js'), read('src/ai.js'), read('src/app.js'),
]);

const script = [analytics, questions, storage, markdown, ai].map(stripExports).concat(stripImports(app)).join('\n\n');
